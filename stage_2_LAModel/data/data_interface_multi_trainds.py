...
class MultiDataSetInterface(pl.LightningDataModule):
    ...
    def setup(self, stage):
        ...
        if stage == 'fit':
            self.trainset_sup, self.collate_fn = self.instancialize(data_mode='train', dataset=self.hparams.dataset_sup)
            self.trainset_real, _ = self.instancialize(data_mode='train', dataset=self.hparams.dataset_real)
            self.valset, _ = self.instancialize(data_mode='val', dataset=self.hparams.dataset_sup)
            logging.info(f"len train_ds_sup: {len(self.trainset_sup)}, len train_ds_real: {len(self.trainset_real)}, len val_ds: {len(self.valset)}")
        elif stage == 'validate' or stage == 'predict':
            self.valset, self.collate_fn = self.instancialize(data_mode='val', dataset=self.hparams.dataset_sup)
        elif stage == 'test':
            self.testset, self.collate_fn = self.instancialize(data_mode='val', dataset=self.hparams.dataset_sup)
