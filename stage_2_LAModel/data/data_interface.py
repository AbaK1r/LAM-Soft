...
class DataInterface(pl.LightningDataModule):
    ...
    def setup(self, stage):
        ...
        if stage == 'fit':
            dataset, self.collate_fn = self.instancialize(data_mode='train', dataset=self.hparams.dataset)
            self.trainset, self.valset = random_split(dataset, [0.8, 0.2], torch.Generator().manual_seed(172))
