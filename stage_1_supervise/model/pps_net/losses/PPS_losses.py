...
def rgb_to_grayscale(input_tensor):
    ...
    return torch.matmul(input_tensor.permute(0, 2, 3, 1),
                        torch.tensor([0.2989, 0.5870, 0.1140], device=input_tensor.device)).unsqueeze(1)
...
def calculate_pps_corr_loss(img_tensor, l, a, normal):
    ...
    dot_product_attenuated_masked = dot_product_attenuated * (
                1 - specular_mask)  # Apply mask to the attenuated dot product
    ...
    dot_product_attenuated_flat_masked = dot_product_attenuated_masked.flatten(
        start_dim=2)  # Flatten spatial dimensions of the masked dot product
    ...
    dot_product_mean_subtracted_masked = dot_product_attenuated_flat_masked - dot_product_attenuated_flat_masked.mean(
        dim=2, keepdim=True)
    ...
    return loss, (1 - specular_mask).bool()
