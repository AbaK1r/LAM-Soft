...
def pixel_intrinsics_to_normalized_intrinsics(intrinsics, image_shape):
    ...
    M = M.to(intrinsics.device).unsqueeze(0).repeat(intrinsics.size(0), 1, 1).float()
...
def get_camera_pixel_directions(image_shape, intrinsics, normalized_intrinsics=True):
    ...
    dirs = pixel_coords_to_directions(pts.repeat(intrinsics.size(0), 1, 1, 1), intrinsics).to(intrinsics.device)
    return dirs
